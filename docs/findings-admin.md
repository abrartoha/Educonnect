...
- P1: reactivateEntity had no role guard. An admin could 
  reactivate a STUDENT or ADMIN account through this endpoint,
  which is not an intended operation. approveEntity already had 
  the correct guard — reactivateEntity now mirrors it.
  Fix: added role select to findUnique, added reactivatableRoles 
  check ['UNIVERSITY', 'AGENT', 'CONSULTANT'], throws 
  BadRequestError for any other role.
